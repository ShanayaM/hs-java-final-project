 ...
 *
 */
public class GUISortBySongTitleButton extends GUIModel implements Command {

    public GUISortBySongTitleButton(List model, GUIInterface view) {
        super(model, view);
    }
    ...
    public void execute(Widget widget) {
        ...
        //Generate text output of the data for 2 views hobby, sorted by song title
        if (SurveyEnum.HOBBY.equals(view.getCurrentType())) {
            generateTextOutput(model);
        }
    }

    public static Comparator<SongInterface> COMPARE_BY_SONG_TITLE = new Comparator<SongInterface>() {
        public int compare(SongInterface one, SongInterface other) {
            return one.getTitle().toLowerCase().compareTo(other.getTitle().toLowerCase());
        }
    };
